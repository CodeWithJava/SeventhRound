public class Solution
{
	public int [] maxNumber(int [] nums1, int [] nums2, int k)
	{
		...
		for(int i = Math.max(k - nums2.length, 0);i <= Math.min(nums1.length, k);i++)
		{
			...
			while(i1 < sub1.length || i2 < sub2.length)
				x[t++] = compare(sub1, sub2, i1, i2) ? sub1[i1++]:sub2[i2++];

			if(compare(x, result, 0, 0))
				result = x;
		}
	...
	private boolean compare(int [] sub1, int [] sub2, int i1, int i2)
	{
		while(i1 < sub1.length && i2 < sub2.length)
		{
	...
	private int [] getMaxSubArray(int [] nums, int k)
	{
		...
		{
			while(p > 0 && p + nums.length - i > k && x[p - 1] < nums[i])
				p--;
