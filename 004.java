public class Solution
{
	public double findMedianSortedArrays(int [] nums1, int [] nums2)
	{
		...
		if(((m + n) & 1) == 0)
			return (findKth(nums1, 0, nums1.length, nums2, 0, nums2.length, k) + findKth(nums1, 0, nums1.length, nums2, 0, nums2.length, k + 1)) / 2;

		return findKth(nums1, 0, nums1.length, nums2, 0, nums2.length, k + 1);
	}
